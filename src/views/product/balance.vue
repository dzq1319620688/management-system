<template>
  <div>
        ...
        >
          <template v-slot="scope">
            ...
            <div v-else-if="v.field === 'status'">
              <el-select v-model="scope.row[v.field]" size="mini" :disabled="scope.row[v.field]==3||scope.row[v.field]==0" @change="confirm(scope.row)">
                <el-option label="未审核" :value="1"></el-option>
                <el-option label="不通过" :value="2"></el-option>
                <el-option label="已结算" :value="3"></el-option>
              </el-select>
    ...
    <el-dialog
      ...
      width="30%"
      >
      <div style="color:#f16767;margin-bottom:20px">确认更改状态，如果更改为已结算，就不能再更改了</div>
      <el-select v-model="form.status" filterable size="medium">
        <el-option :value="1" label="未审核"></el-option>
        <el-option :value="2" label="不通过"></el-option>
        <el-option :value="3" label="已结算"></el-option>
      </el-select>
      <div style="margin: 20px 0 5px;">不通过原因或备注:</div>
      <el-input
        v-model="form.text"
        type="textarea"
        clearable
